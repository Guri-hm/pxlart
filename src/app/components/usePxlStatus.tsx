 ...
 * ストローク保存を扱うためのフック。
 */
export function usePxlStatus(
    defaultValue: CurrentStrokeType
): [strokes: CurrentStrokeType, setStrolkes: (strokes: CurrentStrokeType) => void] {
    ...
    useEffect(() => {
        ...
        if (json !== "") {
            setStrokesInternal(JSON.parse(json) as CurrentStrokeType)
        }
    }, [setStrokesInternal])

    // 外部からのセッター呼び出し時にローカルストレージに値を保存する
    const setStrolkes = useCallback(
        (strokeStrage: CurrentStrokeType) => {
...
export const getStrageStrokes = (): any => {
    let json: string = localStorage.getItem(STORAGE_KEY_ALL_STROKES) || "";
    if (json !== "") {
        return JSON.parse(json) as CurrentStrokeType
    }
    return null;
}
